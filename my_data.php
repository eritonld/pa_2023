...
if(isset($_GET['tahunproses'])){
	$tahunproses = mysqli_real_escape_string($koneksi, $_GET['tahunproses']);
}else{
	$tahunproses=date('Y');
}
...
<script type="text/javascript"> 
	...
	function cekvalid()
	{
		...
		{
			alert("Input employee name");
			document.getElementById('cari').focus();
		}
		else
		{	
			var cari = document.getElementById('cari').value;
			var tahunproses = document.getElementById('tahunproses').value;
			
			if(cari.match(letterNumber)) 
			{
				document.getElementById('proses').style.display = 'inline';
				// xmlhttp.open("GET", "home.php?link=mydata&cari="+cari+"&tahunproses="+tahunproses, true);
				window.location="home.php?link=mydata&cari="+cari+"&tahunproses="+tahunproses;
			}
			else
			{ 
				alert("Try again"); 
					...
					<div class="form-group">
						...
						<div class="col-md-2 col-xs-11">
							<select id="tahunproses" name="tahunproses" class="form-control">
							  ...
							  {?>
								<option value='<?php echo"$t";?>' <?php if($t==isset($tahunproses)) echo"selected";?>><?php echo"$t";?></option>
								<?php
								  }
								?>
							</select>
						...
						<div class="col-md-8 col-xs-11">
							<button class="btn btn-theme" onClick="cekvalid()"><i class="fa fa-search"></i> Search</button>
						</div>
				  </div>
						  ...
						  <section id="no-more-tables">
							  ...
							  <table class="table table-bordered table-striped table-condensed cf">
								  ...
								  <?php
									...
									where tp.input_by = $idmaster_pa and k.Kode_StatusKerja<>'SKH05' and k.Nama_Lengkap like '%$cari%' $qgol order by k.Nama_Lengkap ASC ");
								  ...
								  ?>
									  <tr>
										  ...
										  <td data-title="Total Score"><?php 
												...
												// echo getTotalScorez($r_data[NIK],$r_data[Nama_Golongan],$transaksi_pa)."% (".getGrade(getTotalScorez($r_data[NIK],$r_data[Nama_Golongan],$transaksi_pa)).")";
										  ?></td>
						...
						<section id="no-more-tables">
							  <table class="table table-bordered table-striped table-condensed cf">
								  ...
								  <?php
									...
									$qcekatasan1 = mysqli_query($koneksi,"select  tp.edit_by, tp.edit_komite,tp.date_edit,tp.date_edit2,tp.edit_by2,k.NIK,k.Nama_Lengkap,k.Mulai_Bekerja,dp.Nama_Perusahaan,dep.Nama_Departemen,dg.Nama_Golongan,k.Nama_Jabatan, tp.date_input, do.Nama_OU,tp.total,tpa.total as totalawal,tpa1.total as total1,tpa2.total as total2,
									(Select Nama_Lengkap from $karyawan where nik = (select username from user_pa where id = tp.edit_by))as namaedit1,
									(Select Nama_Lengkap from $karyawan where nik = (select username from user_pa where id = tp.edit_by2))as namaedit2
									 from $karyawan as k
									left join daftarou as do on k.Kode_OU = do.Kode_OU
									...
									where a.nik_atasan1 = '$getnik[username]' and k.Kode_StatusKerja<>'SKH05' and k.Nama_Lengkap like '%$cari%' and tp.date_input is not null order by k.Nama_lengkap ASC");
								  ...
								  ?>
									  <tr>
										  ...
										  <td data-title="Final Total Score"><?php
												...
												// echo getTotalScorez($rcekatasan1[NIK],$rcekatasan1[Nama_Golongan],$transaksi_pa)."% (".getGrade(getTotalScorez($rcekatasan1[NIK],$rcekatasan1[Nama_Golongan],$transaksi_pa)).")";
										  ?></td>
										  <td data-title="Action">
											<?php
										  ...
										  <td data-title="Review">
											<?php if($rcekatasan1['date_edit']<>""){echo "Review";}else{echo "-";}  ?>
										  </td>
									  </tr>
								  <?php
									$no++;
							...
							<section id="no-more-tables">
							  <table class="table table-bordered table-striped table-condensed cf">
								  ...
								  <?php
									...
									left join $transaksi_pa_edit2 as tpa2 on k.NIK = tpa2.NIK
									left join atasan as a on k.NIK = a.nik
									where nik_atasan2 = '$getnik[username]' and k.Kode_StatusKerja<>'SKH05' and k.Nama_Lengkap like '%$cari%'
									and  a.nik_atasan1 <> a.nik_atasan2 and tp.date_input is not null			
								  ...
								  ?>
									  <tr>
										  ...
										  <td data-title="Final Total Score"><?php  
												...
												// echo getTotalScorez($rcekatasan2[NIK],$rcekatasan2[Nama_Golongan],$transaksi_pa)."% (".getGrade(getTotalScorez($rcekatasan2[NIK],$rcekatasan2[Nama_Golongan],$transaksi_pa)).")";
										  ?></td>
										  <td data-title="Action">
											<?php
										  ...
										  <td data-title="Review">
											<?php if($rcekatasan2['date_edit2']<>""){echo "Review";}else{echo "-";}  ?>
										  </td>
									  </tr>
								  <?php
									$no++;

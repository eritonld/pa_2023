...
dg.Nama_Golongan,dj.Nama_Jabatan from $karyawan as k
...
//echo "select komposisi_group,nama_group,jml_loop,fortable,bobot from $tabel_prosedure where fortable='$fortable' order by id";
...
while($cprosedure=mysqli_fetch_array($prosedure))
{
	...
	for($i=1;$i<=$cprosedure['jml_loop'];$i++)
	{
		...
		$master_soal=mysqli_query($koneksi, "select $slc,komentar from master_soal_$fortable_select");
		//echo "<br>select $slc,komentar from master_soal_$fortable_select";
...
// Print a text
//echo "<br>$tabisi";
if(isset($cgetsp['statussp'])){
	$statussp = $cgetsp['statussp'];
	$periode = $cgetsp['periode'];
}else{
	$statussp = "";
	$periode = "";
}
...
<table width="90%" class="border-tipis">
        ...
        <tr>
            ...
            <td class="border-tipis">$statussp / $periode</td>  
        </tr>  
</table>
$tabmgr
<br>

...
$q_data = mysqli_query ($koneksi, "select k.id,k.nik_baru,k.Nama_Lengkap,k.Mulai_Bekerja,dp.Nama_Perusahaan,dep.Nama_Departemen,
...
while ($r_data = mysqli_fetch_array ($q_data))
{
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, "'".$r_data['nik_baru']);
	$cell = 'B'.$row;
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $r_data['total_score']);
	$cell = 'I'.$row;
	$objPHPExcel->getActiveSheet()->getStyle($cell)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);	
	$objPHPExcel->getActiveSheet()->getStyle($cell)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED);
	$objPHPExcel->getActiveSheet()->getStyle($cell)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED);
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, getGrade($r_data['total_score']));
	$cell = 'J'.$row;
	$objPHPExcel->getActiveSheet()->getStyle($cell)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);	
	$objPHPExcel->getActiveSheet()->getStyle($cell)->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED);
	$objPHPExcel->getActiveSheet()->getStyle($cell)->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOTTED);
	...
	$q_sp	= mysqli_query($koneksi, "Select * from $tabel_sp where nik = '$r_data[nik_baru]'");
